...
def get_project_folder(output_path):
    # Assuming the project folder is the second-to-last element in the path
    project_folder = os.path.dirname(output_path)
...
def set_up_render_context(blender_file_path):
    bpy.ops.wm.open_mainfile(filepath=blender_file_path)

    render_frame = bpy.context.scene.frame_current
...
def set_output_path(output_path, render_frame):
    # Establecer la ruta de salida dinámica
    bpy.context.scene.render.filepath = os.path.join(output_path, f"{render_frame:05d}")
...
def main():
    ...
    blender_file_path, output_path = sys.argv[-2:]

    print(blender_file_path)
    print(output_path)

    project_folder = get_project_folder(output_path)
    print(f"Project folder: {project_folder}")
    
    render_frame = set_up_render_context(blender_file_path)

    configure_compositor(output_path)

    set_output_path(output_path, render_frame)
